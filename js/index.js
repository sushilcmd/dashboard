        ...
        $('.loginBtn').bind('click', function() {
            ...
            execute('login', loginInfo).then(function(r) {
                if (!r.status) {
                    ...
                    helperMeta["outletsData"] = {};
                    if (r.outlets.length > 1) {
                        execute('getRunningOrders', { customerName: r.customerName, startDate: "20190901", endDate: "20191210" }).then(data => {
                            $('.loaderContainer').hide();
                            console.log(data);
                            outletScreen.show({ data })
                        })
                    } else {
...
const getOutletSale = function() {
    execute('orders', customerName).then(data => {
        console.log(data);
    })
}
...
var outletScreen = new function() {
    ...
    this.bind = function() {
        $('.outlet').bind('click', function() {
            var dataIndex = $(this).data('index')
            helperMeta["outletID"] = $(this).tmplItem().data.data[dataIndex]._id
            dashboardScreen.show(helperMeta)
        })
